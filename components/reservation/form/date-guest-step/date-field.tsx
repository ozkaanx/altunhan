...
export function DateField({ label, value, min, disabled, onChange }: DateFieldProps) {
  return (
    <label className="block min-w-0">
      ...
      <span
        className={`mt-2 flex h-[54px] w-full min-w-0 items-center overflow-hidden border border-[#D9D5CD] bg-white transition-colors focus-within:border-[#263A2D] ${disabled ? "cursor-not-allowed opacity-60" : "cursor-pointer"} `}
      >
        <span className="pointer-events-none flex h-full w-12 shrink-0 items-center justify-center border-r border-[#EEEAE3]">
