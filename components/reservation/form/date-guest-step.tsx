...
export function DateGuestStep({
  ...
  const capacity = selectedAccommodation?.capacity ?? 1;
    ...
    <section className="border border-[#E3E0D8] bg-white p-4 sm:p-6">
        ...
        <div className="mt-5 border border-[#E7D8C0] bg-[#FAF5EA] p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {busyRanges.slice(0, 8).map((range) => (
              ...
              >
                {formatReservationDate(range.checkIn)} →{" "}
                {formatReservationDate(range.checkOut)}
              </span>
            ))}
          </div>
      ...
      {availabilityError && (
        <div className="mt-5 flex items-start gap-2 border border-[#E7D8C0] bg-[#FAF5EA] p-3 text-xs leading-5 text-[#88662F]">
          ...
          {availabilityError} Rezervasyon gönderilirken müsaitlik tekrar kontrol
          edilecek.
        </div>
      )}
            ...
            <input
              ...
              onClick={(event) => {
                try {
                  ...
                  // iOS Safari native tıklama davranışını kullanır.
                }
              }}
              className="
          h-full
    ...
    `}
            ...
            <input
              ...
              onClick={(event) => {
                try {
                  ...
                  // Safari native picker'ı kullanır.
                }
              }}
              className="
        h-full
      ...
      <div className="mt-5 flex items-center justify-between border-t border-[#EEEAE3] pt-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-medium text-[#40463F]">Misafir Sayısı</p>
        ...
        <div className="flex items-center border border-[#DDD9D1]">
          ...
          <button
            type="button"
            onClick={() =>
              onGuestCountChange(Math.min(capacity, guestCount + 1))
            }
            className="flex h-10 w-10 items-center justify-center"
          >
            <Plus size={14} />
