...
begin
  ...
  if v_reservation.status = 'pending_payment'
     and v_reservation.room_id is not null
     and exists (
       select 1
       from public.reservations conflicting_reservation
       where conflicting_reservation.id <> v_reservation.id
         and conflicting_reservation.room_id = v_reservation.room_id
         and conflicting_reservation.status in ('pending_approval', 'confirmed')
         and conflicting_reservation.check_in < v_reservation.check_out
         and conflicting_reservation.check_out > v_reservation.check_in
     ) then
    raise exception 'Bu oda aynı tarihlerde başka bir rezervasyona verildi. Önce "Odayı Değiştir" ile müsait bir oda seçin, ardından tahsilatı tekrar kaydedin.';
  end if;
...
begin
  ...
  if v_reservation.status = 'pending_payment'
    ...
    raise exception 'Bu oda aynı tarihlerde başka bir rezervasyona verildi. Önce "Odayı Değiştir" ile müsait bir oda seçin, ardından dekontu tekrar onaylayın.';
  end if;
...
revoke all on function public.record_admin_reservation_payment(bigint, numeric, text, text)
  from public, anon;
grant execute on function public.record_admin_reservation_payment(bigint, numeric, text, text)
  to authenticated, service_role;

revoke all on function public.verify_admin_reservation_payment(bigint, numeric)
  from public, anon;
grant execute on function public.verify_admin_reservation_payment(bigint, numeric)
  to authenticated, service_role;
