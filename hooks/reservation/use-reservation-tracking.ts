...
import { findReservation } from "@/app/rezervasyon/takip/action";
...
export function useReservationTracking() {
  ...
  useEffect(() => {
    ...
    const refreshReservation = async () => {
      try {
        const result = await findReservation(trackedReservationCode, phone);
  ...
  }, [trackedReservationCode, reservationStatus, phone]);
