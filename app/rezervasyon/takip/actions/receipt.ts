...
export async function submitTrackedReceipt(
  ...
  if (!code) {
    return {
      success: false,
      message: "Rezervasyon numarası bulunamadı.",
    };
  }
  ...
  const { data, error } = await supabase.rpc("submit_tracked_reservation_receipt", {
  ...
  if (error) {
    console.error("Takip ekranından dekont kaydedilemedi:", error);
  ...
  try {
    await notifyReceiptSubmitted(supabase, code, path);
  } catch (notificationError) {
    console.error("Takip dekont bildirimi gönderilemedi:", notificationError);
  }
